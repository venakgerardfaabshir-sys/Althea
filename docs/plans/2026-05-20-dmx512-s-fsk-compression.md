...
为了在这种极端物理瓶颈下保证系统能够提供顺滑控制并防止崩溃，本计划设计并强制实施以下**五大防瘫痪与自愈安全策略**：
...
2.  **接收端：帧渐变平滑插值引擎 (Frame Interpolation)**
    接收端解压出 2.4Hz 的低频帧后，**不直接输出**（否则肉眼可见严重卡顿），而是在物理 44Hz 定时器驱动下进行**线性插值过渡**（过渡斜坡时间 $\approx 100\text{ms}$），平滑滑入下一状态，从而在视觉上消除跳变感与卡顿。
3.  **发送端/解压端：令牌桶容量下限保障 (Token Bucket Capacity Floor)**
    当用户或信道适配层将带宽限速值调节得非常低（例如 3kbps，即 375 字节/秒）时，限速器容量若低于 513 字节，将导致需要 513 字节的 I-Frame 永远无法通过（令牌桶累积上限小于一帧），从而导致发送端永久死锁。系统强制将**令牌桶的最大容量下限（Capacity Floor）锁定在 513 字节**（即 `Math.max(513, maxBytesPerSec)`），从而在极低物理带宽下仍能通过时间累积正常发送 I-Frame，确保死锁自愈。
4.  **单向物理信道下的周期性关键帧刷新机制 (Periodic I-Frame Refresh)**
    由于 S-FSK 物理链路为单向传输且不具备反向信道，接收端若因环境噪声干扰导致丢包、失步（`syncLost = true`），将永久拒绝解析后续的所有差分 P-Frame（以防产生级联漂移错误）。为了恢复同步，系统在发送端引入**每隔 180 帧（在 44.09Hz 下约为 4.08 秒）强制发送一次全量 I-Frame 的周期性刷新机制**。这保证了在无反馈链路的情况下，接收端能在最长 4.08 秒内自动纠错并恢复同步。
5.  **瞬时自愈控制触发 (Instant Recovery Trigger)**
    在用户手动调节限速滑块（Limiter Rate Slider）或切换测试场景（Scenario Switch）等关键控制交互瞬间，系统会**立即将强制 I-Frame 标记置为真 (`forceIFrameTrigger = true`)**，使发送端能够立刻将最新的全量状态同步到接收端，实现零时延的人机交互自愈。
6.  **防熄灭看门狗 (Keepalive Failsafe)**
